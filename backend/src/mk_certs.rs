...
pub fn mk_request(key_pair: &PKey<Private>) -> Result<X509Req, ErrorStack> {
...
pub fn mk_ca_signed_cert(
    ...
    cert_builder.set_pubkey(&ca_key_pair)?;
    let not_before = Asn1Time::days_from_now(0)?;
    cert_builder.set_not_before(&not_before)?;
    let not_after = Asn1Time::days_from_now(365)?;
    cert_builder.set_not_after(&not_after)?;
    ...
    if let Some(alt_names) = alt_names {
        if !alt_names.is_empty() {
            ...
            let built =
                subject_alt_name.build(&cert_builder.x509v3_context(Some(ca_cert), None))?;
            cert_builder.append_extension(built)?;
        }
    }

    cert_builder.sign(ca_key_pair, MessageDigest::sha256())?;
    req.subject_name().entries().for_each(|entry| {
        println!("entry: {:?}", entry.data());
    });
